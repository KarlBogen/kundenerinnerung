  ...
  function RemoveFromList($key, $mail) {
    if (!xtc_not_null($key) && $this->remove === false) {
      ...
      if (xtc_db_num_rows($check_mail_query) > 0) {
        ...
        xtc_db_perform(TABLE_CUSTOMERS_REMIND_RECIPIENTS, $sql_data_array, 'update', "customers_email_address = '".xtc_db_input($check_mail['customers_email_address'])."'".$where);
        ...
        $this->message = TEXT_EMAIL_DEL_REMINDER;
    ...
    if (!xtc_not_null($key)) {
      ...
      if (xtc_db_num_rows($check_mail_query) > 0) {
        ...
        if($check_mail['mail_status'] == '1') {
          $this->message = TEXT_EMAIL_EXIST_REMINDER;
          $this->message_class = 'error';
